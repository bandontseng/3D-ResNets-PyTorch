...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        # Bandon Added
        self.fc = torch.nn.Linear(self.num_classes, self.num_classes)
...
def load_conv3d(state_dict, name_pt, sess, name_tf, bias=False, bn=True):
    ...
    if bn:
        ...
        # To fix dimension error.
        # The size of model is [64] for example, but the size of the pretrained weight is [1, 1, 1, 1, 64].
        # So that I have to convert the size to match the model.
        moving_mean = moving_mean.reshape(-1)
        moving_var = moving_var.reshape(-1)
        beta = beta.reshape(-1)
